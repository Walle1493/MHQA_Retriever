...
class InputExample(object):
    ...
    def __init__(self, example_id, question, contexts, labels=None):
...
class InputFeatures(object):
    def __init__(self, example_id, choices_features, labels):
        self.example_id = example_id
...
class RetrieverProcessor1(DataProcessor):
    """Retriever Processor 1 for the HotpotQA data set."""
    ...
    def get_dev_examples(self, data_dir):
        ...
        return self._create_examples(self._read_json(os.path.join(data_dir, "dev_dist.json")), "dev")

    def get_test_examples(self, data_dir):
        logger.info("LOOKING AT {} test".format(data_dir))
        # TODO: replace dev set with test set
        return self._create_examples(self._read_json(os.path.join(data_dir, "dev_dist.json")), "test")
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        ...
        labels = []
        for number in example.labels:
            label = label_map[number]
            labels.append(label)
        ...
        if ex_index < 2:
            ...
            for choice_idx, (input_ids, attention_mask, token_type_ids) in enumerate(choices_features):
                ...
                logger.info("label: {}".format(labels))

        features.append(InputFeatures(example_id=example.example_id, choices_features=choices_features, labels=labels,))
